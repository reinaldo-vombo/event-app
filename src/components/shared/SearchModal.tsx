   ...
   return (
      <>
         <main className="h-full center relative w-full">
            ...
            <motion.ul
               className={`flex flex-wrap gap-4 justify-center items-center size-full ${isHorizontal ? "flex-row" : "flex-col"
                  }`}
               layout
...
export function Button(props: { className?: string; children: ReactNode }) {
   return (
      <button
         className={`p-2 border-2 border-[#161616] rounded-full hover:bg-[#161616] flex items-center justify-center text-[#9C9BA1] hover:text-white ${props.className || ""
            }`.trim()}
      >
         {props.children}
      </button>
